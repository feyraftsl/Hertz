package fey.hertzmusic.data.source.local.lyrics
...
private fun readId3(file: File): String? =
    file.inputStream().buffered().use { input ->
        val header = ByteArray(10)
        if (!input.readFully(header)) return null
        if (!header.startsWith("ID3")) return null
...
private fun readFlac(file: File): String? =
    file.inputStream().buffered().use { input ->
        val magic = ByteArray(4)
        if (!input.readFully(magic) || !magic.startsWith("fLaC")) return null
...
private fun readMp4(file: File): String? =
...
object LyricsExtractor {
    ...
    fun extract(path: String, mime: String): String? =
        runCatching {
            ...
            when {
                magic.startsWith("ID3") -> readId3(file)
                magic.startsWith("fLaC") -> readFlac(file)
